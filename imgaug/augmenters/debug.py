...
class _ListOfArraysStats(object):
    ...
    def get_channels_min(self, default):
        if self.empty:
            return -1
        elif any([arr.ndim == 2 for arr in self.arrays]):
            return default
    ...
    def get_channels_max(self, default):
        if self.empty:
            return -1
        elif not any([arr.ndim > 2 for arr in self.arrays]):
            return default
    ...
    def unique_dtype_names(self):
        return sorted(list(set([arr.dtype.name for arr in self.arrays])))
...
class SaveDebugImageEveryNBatches(_SaveDebugImage):
    ...
    def __init__(self, destination, interval,
        ...
        if isinstance(destination, _IImageDestination):
